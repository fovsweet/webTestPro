    ...
    jm.util = {
        ...
        canvas:{
            bezierto: function(ctx,x1,y1,x2,y2){
                ctx.beginPath();
                ctx.moveTo(x1-13,y1+0.5);
                var harfLine = x1+((x2-x1)/2)+0.5;
                if(y2-y1>0){
                    ctx.lineTo(harfLine,y1+0.5);
                    ctx.lineTo(harfLine,y2+0.5-5);
                    ctx.arc(harfLine+5,y2+0.5-5,5,Math.PI,0.5*Math.PI,true);
                    ...
                    ctx.lineTo(harfLine,y1+0.5);
                    ctx.lineTo(harfLine,y2+0.5+5);
                    ctx.arc(harfLine+5,y2+0.5+5,5,Math.PI,1.5*Math.PI,false);
                    ctx.lineTo(x2+0.5,y2+0.5);
                }
                ctx.stroke();
            },
...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        create_node_element:function(node,parent_node){
            ...
            d.setAttribute('nodeLeaf',node.data.leaf);
            d.setAttribute('nodeType',node.data.type);
            d.setAttribute('nodeRecordId',node.data.recordId);
            d.setAttribute('nodeCode',node.data.code);
            d.setAttribute('nodeParentCode',node.data.parentCode);
            d.setAttribute('nodeVersion',node.data.version);
